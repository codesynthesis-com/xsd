    ...
    generate_serialization_source (Context& ctx, size_t first, size_t last)
    {
      ...
      if (!elemen_type)
        ctx.os << "#include <ostream>" << endl
               << "#include <libxsd/cxx/tree/error-handler.hxx>" << endl;

      ctx.os << "#include <libxsd/cxx/xml/dom/serialization-source.hxx>" << endl
             << endl;

      if (ctx.polymorphic)
      {
        ctx.os << "#include <libxsd/cxx/tree/type-serializer-map.hxx>" << endl
               << endl;
