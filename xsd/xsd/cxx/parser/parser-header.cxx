  ...
  namespace Parser
  {
    namespace
    {
      ...
      struct FundNamespace: Namespace, Context
      {
        ...
        traverse (Type& ns)
        {
          ...
          os << "// Exceptions. See libxsd/cxx/parser/exceptions.hxx " <<
            "for details." << endl
             << "//" << endl
          ...
          os << "// Error handler. See " <<
            "libxsd/cxx/xml/error-handler.hxx for details." << endl
             << "//" << endl
          ...
          if (xml_parser == L"xerces")
          {
            os << "// Parsing flags. See " <<
              "libxsd/cxx/parser/xerces/elements.hxx" << endl
               << "// for details." << endl
               << "//" << endl
          ...
          {
            os << "// Document type. See " <<
              "libxsd/cxx/parser/expat/elements.hxx" << endl
               << "// for details." << endl
               << "//" << endl
    ...
    generate_parser_header (Context& ctx, bool generate_xml_schema)
    {
      ...
      else
      {
        ...
        {
          ctx.os << "#include <libxsd/cxx/xml/char-" << ctx.char_encoding << ".hxx>" << endl;
        }

        ctx.os << "#include <libxsd/cxx/xml/error-handler.hxx>" << endl
               << "#include <libxsd/cxx/parser/exceptions.hxx>" << endl
               << "#include <libxsd/cxx/parser/elements.hxx>" << endl
               << "#include <libxsd/cxx/parser/xml-schema.hxx>" << endl;

        if (ctx.polymorphic)
          ctx.os << "#include <libxsd/cxx/parser/map.hxx>" << endl;

        if (ctx.validation)
          ctx.os << "#include <libxsd/cxx/parser/validating/parser.hxx>" << endl
                 << "#include <libxsd/cxx/parser/validating/exceptions.hxx>" << endl
                 << "#include <libxsd/cxx/parser/validating/xml-schema-pskel.hxx>" << endl
                 << "#include <libxsd/cxx/parser/validating/xml-schema-pimpl.hxx>" << endl;
        else
          ctx.os << "#include <libxsd/cxx/parser/non-validating/parser.hxx>" << endl
                 << "#include <libxsd/cxx/parser/non-validating/xml-schema-pskel.hxx>" << endl
                 << "#include <libxsd/cxx/parser/non-validating/xml-schema-pimpl.hxx>" << endl;

        ctx.os << "#include <libxsd/cxx/parser/" << ctx.xml_parser <<
          "/elements.hxx>" << endl
               << endl;
